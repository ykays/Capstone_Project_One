...
def get_user_templates(user_id):
    users_templates = [template.serialize() for template in ListTemplate.query.filter(
        ListTemplate.user_id == user_id).all()]
...
def get_template_products(id):
    all_products = db.session.query(Product.id, Product.product_name, ProductCategory.category_name).join(
        TemplateProduct).join(ProductCategory).filter(TemplateProduct.template_id == id).all()
    return [product._asdict() for product in all_products]


def delete_template_product(user_id, id):
    template = ListTemplate.query.filter(
        ListTemplate.user_id == user_id).first()
...
def add_template_product(user_id, id):
    template = ListTemplate.query.filter(
        ListTemplate.user_id == user_id).first()
    product_exist_check = TemplateProduct.query.filter(
        (TemplateProduct.template_id == template.id) & (TemplateProduct.product_id == id)).all()
