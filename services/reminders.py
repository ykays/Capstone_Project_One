...
def add_reminder(username, product_id, quantity):
    ...
    new_reminder = Reminder(user_id=user_id, product_id=product_id, quantity=new_quantity)
...
def get_reminders(username):
    user_id = get_user_id(username)
    reminders = []
    all_reminders = db.session.query(Product.id, Product.product_name, ProductCategory.category_name, Reminder.id, Reminder.user_id, Reminder.product_id, Reminder.quantity).join(Reminder).join(ProductCategory).filter(Reminder.user_id == user_id).all()
