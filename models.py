...
class User(db.Model):
    ...
    grocery_lists = db.relationship(
        'GroceryList', cascade="all, delete-orphan")
...
class ListTemplate(db.Model):
    ...
    template_products = db.relationship(
        'TemplateProduct', cascade="all, delete-orphan")
...
class Product(db.Model):
    ...
    template_products = db.relationship(
        'TemplateProduct', cascade="all, delete-orphan")
    grocery_list_products = db.relationship(
        'GroceryListProducts', cascade="all, delete-orphan")
...
class GroceryListProducts(db.Model):
    ...
    grocery_list_id = db.Column(db.Integer, db.ForeignKey(
        'grocery_lists.id', ondelete='CASCADE'), nullable=False)
    product_id = db.Column(db.Integer, db.ForeignKey(
        'products.id', ondelete='CASCADE'), nullable=False)
    quantity = db.Column(db.Integer, nullable=False, default=1)
